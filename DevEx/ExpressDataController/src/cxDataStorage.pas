...
{$ENDIF}
  ...
  public
    ...
    class function IsValueValid(const Value: Variant): Boolean; virtual;
...
{$IFDEF DELPHI6}
  ...
  public
    constructor Create(AValueDefs: TcxValueDefs; AValueTypeClass: TcxValueTypeClass);
  ...
  public
    ...
    procedure ReadRecord(ARecordIndex: Integer; AValueDefReader: TcxValueDefReader; AExceptInternal: Boolean);
...
class function TcxValueType.IsValueValid(const Value: Variant): Boolean;
var
  V: Variant;
begin
...
//      Result := CompareStr(PStringValue(P1)^, PStringValue(P2)^);
      if PStringValue(P1)^ = PStringValue(P2)^ then
        Result := 0
      else
        if PStringValue(P1)^ < PStringValue(P2)^ then
          Result := -1
...
begin
  ...
  P := PPointer(PBuffer)^;
  if P <> nil then
    WriteStringProc(AStream, P^)
  else
    WriteStringProc(AStream, '');
end;
...
begin
  ...
  begin
    if Assigned(P2) then
    begin
//      Result := CompareStr(PWideStringValue(P1)^, PWideStringValue(P2)^);
      if PWideStringValue(P1)^ = PWideStringValue(P2)^ then
        Result := 0
      else
        if PWideStringValue(P1)^ < PWideStringValue(P2)^ then
          Result := -1
...
begin
  ...
  P := PPointer(PBuffer)^;
  if P <> nil then
    WriteWideStringProc(AStream, P^)
  else
    WriteWideStringProc(AStream, '');
end;
...
begin
  PSmallInt(PBuffer)^ := ReadSmallIntFunc(AStream);
end;
...
begin
  WriteSmallIntProc(AStream, PSmallInt(PBuffer)^);
end;
...
begin
  PInteger(PBuffer)^ := ReadIntegerFunc(AStream);
end;
...
begin
  WriteIntegerProc(AStream, PInteger(PBuffer)^);
end;
...
begin
  PWord(PBuffer)^ := ReadWordFunc(AStream);
end;
...
begin
  WriteWordProc(AStream, PWord(PBuffer)^);
end;
...
//const
//  Booleans: array[Boolean, Boolean] of Integer = ((0, -1), (1, 0));
begin
//  Result := Booleans[PBoolean(P1)^, PBoolean(P2)^];
...
begin
  PBoolean(PBuffer)^ := ReadBooleanFunc(AStream);
end;
...
begin
  WriteBooleanProc(AStream, PBoolean(PBuffer)^);
end;
...
begin
  AStream.ReadBuffer(PDouble(PBuffer)^, GetDataSize);
//  PDouble(PBuffer)^ := ReadFloatFunc(AStream);
end;
...
begin
  AStream.WriteBuffer(PDouble(PBuffer)^, GetDataSize);
//  WriteFloatProc(AStream, PDouble(PBuffer)^);
end;
...
begin
  if PCurrency(P1)^ = PCurrency(P2)^ then
    Result := 0
  else
    if PCurrency(P1)^ < PCurrency(P2)^ then
      Result := -1
...
begin
  PCurrency(PBuffer)^ := ReadCurrencyFunc(AStream);
end;
...
begin
  WriteCurrencyProc(AStream, PCurrency(PBuffer)^);
end;
...
begin
  if PDateTime(P1)^ = PDateTime(P2)^ then
    Result := 0
  else
    if PDateTime(P1)^ < PDateTime(P2)^ then
      Result := -1
...
begin
  if PLargeInt(P1)^ = PLargeInt(P2)^ then
    Result := 0
  else
    if PLargeInt(P1)^ < PLargeInt(P2)^ then
      Result := -1
...
begin
  PLargeInt(PBuffer)^ := ReadLargeIntFunc(AStream);
end;
...
begin
  Result := PSQLTimeStamp(P1)^.Year - PSQLTimeStamp(P2)^.Year;
  if Result = 0 then
  begin
    Result := PSQLTimeStamp(P1)^.Month - PSQLTimeStamp(P2)^.Month;
    if Result = 0 then
    begin
      Result := PSQLTimeStamp(P1)^.Day - PSQLTimeStamp(P2)^.Day;
      if Result = 0 then
      begin
        Result := PSQLTimeStamp(P1)^.Hour - PSQLTimeStamp(P2)^.Hour;
        if Result = 0 then
        begin
          Result := PSQLTimeStamp(P1)^.Minute - PSQLTimeStamp(P2)^.Minute;
          if Result = 0 then
          begin
            Result := PSQLTimeStamp(P1)^.Second - PSQLTimeStamp(P2)^.Second;
            if Result = 0 then
              Result := PSQLTimeStamp(P1)^.Fractions - PSQLTimeStamp(P2)^.Fractions;
          end;
        end;
      end;
    end;
...
begin
  if IsNullValue(P1 + Offset) then
  begin
    if IsNullValue(P2 + Offset) then
      Result := 0
    else
      Result := -1
  end
  ...
  begin
    if IsNullValue(P2 + Offset) then
      ...
      Result := ValueTypeClass.Compare(P1 + Offset + ValueFlagSize, P2 + Offset + ValueFlagSize);
  end;
end;
...
begin
  ...
  if not IsNullValue(PCurrent) then
    ValueTypeClass.FreeBuffer(PCurrent + ValueFlagSize);
  if TextStored then
    FreeTextBuffer(PCurrent + ValueFlagSize + DataSize);
end;

procedure TcxValueDef.FreeTextBuffer(PBuffer: PChar);
begin
  if PPointer(PBuffer)^ <> nil then
    Dispose(PStringValue(PPointer(PBuffer)^));
end;

function TcxValueDef.GetDataValue(PBuffer: PChar): Variant;
begin
  if IsNullValue(PBuffer + Offset) then
    Result := Null
  else
    Result := ValueTypeClass.GetDataValue(PBuffer + Offset + ValueFlagSize);
end;

function TcxValueDef.GetDisplayText(PBuffer: PChar): string;
begin
  if TextStored then
    Result := ValueTypeClass.GetDisplayText(PBuffer + Offset + ValueFlagSize + DataSize)
  else
  begin
    if IsNullValue(PBuffer + Offset) then
      Result := ''
    else
      Result := ValueTypeClass.GetDefaultDisplayText(PBuffer + Offset + ValueFlagSize);
  end;
end;
...
begin
  if ReadNullFlag then
    SetNull(PBuffer + Offset, True)
  else
  begin
    SetNull(PBuffer + Offset, False);
    ValueTypeClass.ReadDataValue(PBuffer + Offset + ValueFlagSize, AStream);
  end;
end;
...
begin
  if TextStored then
    ValueTypeClass.ReadDisplayText(PBuffer + Offset + ValueFlagSize + DataSize, AStream);
end;
...
begin
  ...
  begin
    SetNull(PBuffer + Offset, False);
    ValueTypeClass.SetDataValue(PBuffer + Offset + ValueFlagSize, Value);
  end;
end;
...
begin
  if TextStored then
    ValueTypeClass.SetDisplayText(PBuffer + Offset + ValueFlagSize + DataSize, DisplayText);
end;
...
begin
  ...
  begin
    ...
    begin
      ValueTypeClass.FreeBuffer(PBuffer + ValueFlagSize);
...
begin
  ...
  begin
    WriteNullFlag(False);
    ValueTypeClass.WriteDataValue(PBuffer + Offset + ValueFlagSize, AStream);
  end;
end;
...
begin
  if TextStored then
    ValueTypeClass.WriteDisplayText(PBuffer + Offset + ValueFlagSize + DataSize, AStream);
end;
...
begin
  ...
  begin
    ...
    if AExceptInternal and AValueDefReader.IsInternal(AValueDef) then
      Continue;
...
procedure TcxDataStorage.WriteData(ARecordIndex: Integer; AStream: TStream);
  ...
  procedure WriteNilFlag(ANil: Boolean);
  begin
    WriteBooleanProc(AStream, ANil);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        PSource := PBuffer + AValueDefs[AIndex].Offset;
      ...
      begin
        ...
        if AValueDefs.Count > 0 then
          PSource := PSource + AValueDefs[AValueDefs.Count - 1].Offset +
            AValueDefs[AValueDefs.Count - 1].BufferSize
        else
          PSource := PSource + AValueDefs.RecordSize;
      end;
    ...
    if PBuffer <> nil then
      if AFreeAndNil then
        ...
        begin
          ...
          PDest := PBuffer + AValueDef.Offset;
...
begin
  if Index >= 0 then
    Result := FRecordBuffers[Index]
  else
    Result := FInternalRecordBuffers[-Index - 1];
end;
