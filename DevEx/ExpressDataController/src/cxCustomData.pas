...
uses
  ...
  cxDataUtils, cxDataStorage, cxFilter;
...
//    function GetRecordCount: Integer; virtual;
    ...
    function GetValueDefReaderClass: TcxValueDefReaderClass; virtual; abstract;
  ...
  public
    constructor Create(ARelationList: TcxCustomDataRelationList; AItem: TObject);
    destructor Destroy; override;
  ...
  private
    ...
    procedure QuickSortRecords(ASortList: PPointerList; L, R: Integer);
  ...
  private
    ...
    FRecordIndex: Integer;
  public
    constructor Create(ADataController: TcxCustomDataController; ARecordIndex: Integer);
...
//    function GetDisplayText(ARecordIndex, AItemIndex: Integer): string; virtual;
  ...
  public
    ...
    function GetRowIndexByRecordIndex(ARecordIndex: Integer; AMakeVisible: Boolean): Integer;
    function GetRowInfo(ARowIndex: Integer): TcxRowInfo;
    function GetRowDisplayText(const ARowInfo: TcxRowInfo; AItemIndex: Integer): string;
    ...
    procedure ClearSorting(AKeepGroupedItems: Boolean);
    function GetItemSortByDisplayText(AItemIndex: Integer): Boolean; virtual;
...
const
  ...
  stEndText = ')';
  stSeparator = ',';
  NullRecordHandle = TcxDataRecordHandle(-1);
...
type
  TcxInternalValueDefAccess = class(TcxInternalValueDef);
...
{
function TcxCustomDataSource.GetChildCount(AParentHandle: TcxDataRecordHandle): Integer;
begin
  Result := 0;
end;

function TcxCustomDataSource.GetChildRecordHandle(AParentHandle: TcxDataRecordHandle;
  AChildIndex: Integer): TcxDataRecordHandle;
begin
  Result := NullRecordHandle;
end;

function TcxCustomDataSource.GetRootRecordHandle: TcxDataRecordHandle;
begin
  Result := NullRecordHandle;
end;
}
...
begin
  ...
  try
    for I := AList.Count - 1 downto 0 do
      DataController.DeleteRecord(Integer(AList[I]));
  finally
    DataController.EndUpdate;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Result.Free;
        Result := nil;
        DataController.SetStoredValue(ARecordIndex, FDataField, Integer(Result));
      end;
      Exit;
    end;
    if Result = nil then
    ...
    begin
      ...
      begin
        try
          ...
          if eisExpanded in FExpandingInfo.SaveStates then
            DataGroups.Rebuild;
          RebuildSelection;
        end;
      end
...
procedure TcxCustomDataControllerInfo.QuickSortRecords(ASortList: PPointerList;
  L, R: Integer); // see Classes.QuickSort
var
  I, J: Integer;
  P, T: Pointer;
begin
...
function TcxDataFilterCriteriaItem.GetDataValue(AData: Pointer): Variant;
begin
...
begin
  ...
  Filer.DefineBinaryProperty('Criteria', ReadData, WriteData, True);
end;

function TcxDataFilterCriteria.DoFilterRecord(ARecordIndex: Integer): Boolean;
begin
  Result := DoFilterData(Pointer(ARecordIndex));
end;
...
begin
  if not FDestroying and
//    not (csLoading in DataController.FOwner.ComponentState) then // TODO: method IsLoading/Destroying
    ([csDestroying, csLoading] * DataController.FOwner.ComponentState = []) then
  begin
    Prepare;
...
function TcxDataSummaryItem.FormatValue(const AValue: Variant; AIsFooter: Boolean): string;
const
  ACountFormat: array [Boolean] of string = ('COUNT=0', '0');
var
  S: string;
begin
  if not VarIsNull(AValue) then
...
//*
      if Format = '' then
      begin
        if Kind = skCount then
        begin
          S := ACountFormat[AIsFooter];
          Result := FormatFloat(S, AValue);
        end
        else
        begin
          if VarType(AValue) = varDate then
            Result := S + FormatDateTimeSummaryValue(AValue)
          else
            if VarType(AValue) = varCurrency then
              Result := FormatCurrencySummaryValue(AValue)
            else
              Result := FormatFloatSummaryValue(AValue);
        end;
      end
      else
      begin
      ...
      DataController.FormatDateTimeValue(AValue)
    ...
    begin
      ...
      begin
        if AGroupIndex <> -1 then // close summary
          EndCalculateSummary(ASummaryItems, ACountValues, ASummaryValues, ADataGroups[AGroupIndex]^.SummaryValues);
      ...
      begin
        CalculateSummary(ASummaryItems, ADataGroups[ACurIndex]^.BeginRecordListIndex,
          ...
          ADataGroups[ACurIndex]^.SummaryValues);
      end;
      Inc(ACurIndex);
    end;
    if AGroupIndex <> -1 then // close summary
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        if not DataController.IsPattern then
...
begin
  ...
  begin
    ...
    if (ASummaryItems <> nil) and (PSummaryValues <> nil) and not VarIsEmpty(PSummaryValues^) then
...
begin
  if AValueDef is TcxInternalValueDef then
    AValueDef := TcxInternalValueDefAccess(AValueDef).GetValueDef;
...
begin
  ...
  begin
    P := FDataChangedListeners[I];
    if @P^ = @ADataChangedEvent then
    begin
      Dispose(P);
...
begin
  ...
  try
    ...
    begin
      RemoveListenerLink(@ADataController);
...
function TcxCustomDataController.GetRowDisplayText(const ARowInfo: TcxRowInfo; AItemIndex: Integer): string;
begin
...
function TcxCustomDataController.GetItemSortByDisplayText(AItemIndex: Integer): Boolean;
begin
  Result := False;
end;
...
begin
  // TODO: TcxEventList?
  for I := 0 to FDataChangedListeners.Count - 1 do
    Dispose(PNotifyEvent(FDataChangedListeners[I]));
...
begin
  ...
  begin
    if (FDataChangeInfo.Kind in [dcField, dcRecord]) and
      (FDataChangeInfo.RecordIndex <> -1) then
...
procedure TcxCustomDataController.AddListenerLink(ALink: Pointer);
begin
  if FListenerLinks.IndexOf(ALink) = -1 then
    FListenerLinks.Add(ALink);
end;

procedure TcxCustomDataController.RemoveListenerLink(ALink: Pointer);
begin
...
var
  AMasterDataController: TcxCustomDataController;
begin
  if (FDataChangeInfo.RecordIndex <> -1) and (FDataChangeInfo.RecordIndex <> ARecordIndex) then
...
var
  V1, V2: Variant;
begin
  if Assigned(FOnCompare) or IsConversionNeededForCompare(AField) then
  begin
...
function TcxCustomDataController.FindRecordByFields(ABufferRecordIndex: Integer;
  ...
  begin
    ...
    for I := 0 to AFields.Count - 1 do
      if CompareByField(ARecordIndex, ABufferRecordIndex, AFields[I]) <> 0 then
      begin
        Result := False;
        Break;
      end;
...
function TcxCustomDataController.GetComparedValue(ARecordIndex: Integer;
  AField: TcxCustomDataField): Variant;
begin
  if (AField.FNeedConversionFlag = 2) {or
    ((AField.FNeedConversionFlag = 0) and IsConversionNeededForCompare(AField) and
    IsSortByDisplayTextNeeded(AField))} then
    Result := GetDisplayText(ARecordIndex, AField.Index)
  else
    Result := GetInternalValue(ARecordIndex, AField);
//  if GetItemValueSource(AField.Index) = evsValue then
//    Result := GetDisplayText(ARecordIndex, AField.Index)
//  else
//    Result := GetInternalValue(ARecordIndex, AField);
end;
...
begin
  Result := (dcoSortByDisplayText in Options) and not FInternalFindRecord and
    not AField.IsInternal and GetItemSortByDisplayText(AField.Index);
end;
...
begin
  try
    ...
    if (FDataChangeInfo.Kind = dcField) and (FDataChangeInfo.ItemIndex = -1)  then
      FDataChangeInfo.Kind := dcRecord;
    if (FDataChangeInfo.Kind in [dcField, dcRecord]) and (FDataChangeInfo.RecordIndex = -1) then
      FDataChangeInfo.Kind := dcTotal;
