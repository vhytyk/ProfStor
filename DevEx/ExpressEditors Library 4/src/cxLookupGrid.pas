...
uses
  ...
  Types,
  {$ENDIF}
  SysUtils, Classes, Controls, Graphics, Forms, StdCtrls, ExtCtrls,
...
type
  ...
  public
    ...
    constructor Create(AGrid: TcxCustomLookupGrid);
    destructor Destroy; override;
  ...
  TcxLookupGridColumn = class(TCollectionItem, IUnknown, IcxEditRepositoryItemListener)
  private
    FCaption: string;
    ...
    function GetDataController: TcxLookupGridDataController;
    function GetGrid: TcxCustomLookupGrid;
  ...
  protected
    ...
    property DataController: TcxLookupGridDataController read GetDataController;
  public
  ...
  private
    ...
    procedure SetDataController(Value: TcxLookupGridDataController);
  ...
  public
    ...
    property DataController: TcxLookupGridDataController read GetDataController write SetDataController;
...
begin
  ...
  TcxCustomEditStyleAccess(FStyle).ButtonTransparency := ebtHideInactive;
  Result := FStyle;
end;
...
begin
  ...
  try
    ...
    begin
      MaxLineCount := 0;
      Width := -1; // AutoSize is False
    end;
    Result := 2 * cxGridEditOffset +
      AEditViewData.GetEditSize(Canvas, Null, AEditSizeProperties).cy;
  finally
...
procedure TcxLookupGridViewInfo.CalculateCells(ARowViewInfo: TcxLookupGridRowViewInfo);
  ...
  begin
    ...
    CalcCellColors(ASelected and ARowViewInfo.IsFocused,
      ASelected and ACellViewInfo.IsFocused,
      Columns[ACellViewInfo.Index].Style.StyleData.Color,
      Columns[ACellViewInfo.Index].Style.StyleData.FontColor);
    // Calculate
...
begin
  ...
  AColumnViewInfo.Style.Font := AColumn.GetContentFont;
end;
...
begin
  with TcxCustomEditPropertiesAccess(AColumn.Properties)do
  begin
    LockUpdate(True);
    try
    ...
    begin
      ...
      begin
        ...
        LFPainterClass.DrawHeader(Canvas, R, ATextRect, Columns[I].Neighbors, Columns[I].Borders,
          ...
          GetHeaderFont, GetHeaderFont.Color, GetHeaderColor);
        if ASortOrder <> soNone then
...
begin
  ...
  if AGrid <> nil then
  begin
    if not (csDestroying in AGrid.ComponentState) then
      AGrid.BeginUpdate;
  ...
  try
    ...
    if (AGrid <> nil) and not (csDestroying in AGrid.ComponentState) then
      AGrid.EndUpdate;
  end;
end;
...
function TcxLookupGridColumn.GetDataController: TcxLookupGridDataController;
begin
  Result := TcxLookupGridDataController(Grid.FDataController)
end;
...
function TcxCustomLookupGrid.GetDataController: TcxLookupGridDataController;
begin
  Result := TcxLookupGridDataController(FDataController);
end;
...
procedure TcxCustomLookupGrid.SetDataController(Value: TcxLookupGridDataController);
begin
