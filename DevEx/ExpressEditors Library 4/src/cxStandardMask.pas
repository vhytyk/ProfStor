...
procedure TcxStandardMask.Format(var AText: string; AChangeCharCase: Boolean);
var
  I, J: Integer;
  AResult: string;
  AIsPosTested: array of Boolean;
  AChar: Char;
begin
  AResult := FullEmptyString;

  SetLength(AIsPosTested, Length(AResult));
  for I := 0 to Length(AIsPosTested) - 1 do
    AIsPosTested[I] := False;
  ...
  begin
    ...
    begin
      if AIsPosTested[J] then
        Continue;
      if Items[J].Check(AChar) then
      begin
        if not AIsPosTested[J] then
        begin
...
begin
  ...
  begin
    ...
    for I := 1 to Length(AText) do
    begin
      if not Items[I - 1].Check(AText[I]) then
      begin
