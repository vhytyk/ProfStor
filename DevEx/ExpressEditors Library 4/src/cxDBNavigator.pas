...
type
  ...
  protected
    ...
    property Bookmark: Pointer read FBookmark write SetBookmark;
...
{$IFDEF VCL}
  Windows,
{$ENDIF}
  SysUtils, Controls, DBCtrls, Dialogs, cxClasses, cxFilterControlDialog;
...
begin
  AFilterControl := TcxDBFilterControl(Sender);
  ClearFilterCriteria;
  TcxDBFilterControlAccess(AFilterControl).SaveToStream(FFilterCriteria);
  FFilterSetting := True;
  try
    ...
    begin
      DataSet.Filter := AFilterControl.FilterText;
  ...
  case ADefaultIndex of
    ...
    NBDI_DELETE:
      if not ConfirmDelete or
          (MessageDlg(cxNavigator_DeleteRecordQuestion, mtConfirmation, mbOKCancel, 0) = {$IFDEF VCL}ID_OK{$ELSE}mrOk{$ENDIF}) then
        DataSet.Delete;
    NBDI_EDIT: DataSet.Edit;
    NBDI_POST: DataSet.Post;
    NBDI_CANCEL: DataSet.Cancel;
    NBDI_SAVEBOOKMARK: Bookmark := DataSet.GetBookmark;
    NBDI_GOTOBOOKMARK: DataSet.GotoBookmark(FBookmark);
...
begin
  ...
  begin
    case ADefaultIndex of
      ...
      NBDI_INSERT: Result :=  DataSet.CanModify;
      ...
      NBDI_GOTOBOOKMARK: Result := FBookmark <> nil;
      NBDI_REFRESH: Result := DataSet.CanModify;
    end;
  end;
end;
...
begin
  if (FBookmark <> nil) and not DataSet.BookmarkValid(FBookmark) then
    ReleaseBookmark;
end;
...
procedure TcxCustomDBNavigator.DoApplyFilter(Sender: TObject; AFilterControl: TcxDBFilterControl);
begin
  if Assigned(FOnApplyFilter) then
    FOnApplyFilter(Self, AFilterControl);
end;

procedure TcxCustomDBNavigator.DoShowFilterDialog(Sender: TObject; AFilterControl: TcxDBFilterControl);
begin
