...
{$ELSE}
  Types,
{$ENDIF}
  SysUtils, Classes, Controls, Forms, StdCtrls, cxClasses, cxEdit,
  ExtCtrls;
...
begin
  Result := nil;
  with TcxSelectRepositoryItem.Create(nil) do
  try
    Setup(ASetupData);
    if ShowModal = mrOk then
      with lbItems do
        Result := Pointer(Items.Objects[ItemIndex]);
    SaveSetup(ASetupData);
  finally
    Release
  end;
end;
...
begin
  ...
  if btOk.Enabled then
    lbHint.Caption :=
      FList.GetHintByClass(Pointer(lbItems.Items.Objects[lbItems.ItemIndex]))
  else
    lbHint.Caption := '';
end;
...
begin
  ...
  begin
    ...
    for I := 0 to List.Count - 1 do
      lbItems.Items.AddObject(List.Descriptions[I], Pointer(List[I]));
