...
type
  ...
  TcxCustomLookupEditLookupData = class(TcxInterfacedPersistent, IcxTextEditLookupData)
  private
    FOwner: TPersistent;
    FVisible: Boolean;
  ...
  protected
    ...
    procedure LookupGridDropedDown(const AFindStr: string); virtual;
  ...
  public
    ...
    function InternalFocused: Boolean; override;
...
function TcxCustomLookupEditLookupData.CanResizeVisualArea(var NewSize: TSize): Boolean;
begin
...
begin
  ...
  try
    ...
    Properties.LookupGridInitLookAndFeel(Edit.PopupControlsLookAndFeel, Edit.Style.Color, Edit.Style.Font);
...
procedure TcxCustomLookupEditLookupData.DroppedDown(const AFindStr: TCaption);
begin
  Properties.LookupGridDropedDown(AFindStr);
end;
...
begin
  ...
  if GetEditValueSource(False) = evsValue then
    Result := Result + [esoSortingByDisplayText];
end;
...
function TcxCustomLookupEditProperties.GetLookupGridNearestPopupHeight(AHeight: Integer): Integer;
begin
  Result := AHeight;
end;
...
procedure TcxCustomLookupEditProperties.LookupGridDropedDown(const AFindStr: string);
begin
end;
...
begin
  if DataController <> nil then
    DataController.RemoveDataChangedListener(GridDataChanged);
end;
...
begin
  if DataController <> nil then
    DataController.AddDataChangedListener(GridDataChanged);
end;
...
constructor TcxCustomLookupEdit.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Width := 145;
end;
...
function TcxCustomLookupEdit.InternalFocused: Boolean;
begin
  Result := True;
end;
  ...
  if Properties.IsPickMode then
    ...
    if ILookupData.GetDisplayText(ILookupData.CurrentKey) = ADisplayValue then
      ...
      if LookupData.LocateText(ADisplayValue) then
        ...
        EditValue := Properties.GetNullKey;
end;
...
procedure TcxCustomLookupEdit.PropertiesChanged(Sender: TObject);
begin
  if not Focused or not ModifiedAfterEnter then
  begin
    SynchronizeDisplayValue;
//    inherited SynchronizeDisplayValue;
//    if not Properties.IsPickMode then
//      SetInternalDisplayValue(ILookupData.GetDisplayText(EditValue));
  end;
...
begin
  TcxCustomLookupEditProperties(Properties).UnlinkLookupGridControlParent;
end;
...
begin
  ...
  if HasPopupWindow then
    ILookupData.CurrentKey := EditValue;
  if not Properties.IsPickMode then
    SetInternalDisplayValue(ILookupData.GetDisplayText(EditValue{ILookupData.CurrentKey}));
end;
...
begin
  Result := TcxCustomLookupEditProperties(InternalGetProperties);
end;
