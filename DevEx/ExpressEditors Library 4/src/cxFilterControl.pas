...
type
  ...
  TcxCustomFilterControl = class(TcxControl, IcxMouseTrackingCaller, IcxFormatContollerListener)
  private
    FActionMenu: TPopupMenu;
    FComboBox: TcxComboBox;
    FCriteria: TcxFilterControlCriteria;
    FFocused: TcxFilterControlHitTestInfo;
    ...
    procedure RowNavigate(AElement: TcxFilterControlHitTest);
    ...
    procedure ValueEditorFocusChanged(Sender: TObject);
  ...
  public
    constructor Create(AOwner: TcxCustomFilterControl);
    property Control: TcxCustomFilterControl read FControl;
...
{$IFDEF DELPHI6}
  Variants,
{$ENDIF}
  cxVariants, cxFilterConsts, cxFilterControlStrs;
...
var
  ...
  HalftoneBrush: {$IFDEF VCL}HBRUSH{$ELSE}TBrush{$ENDIF};

type
  TControlAccess = class(TControl);
  TWinControlAccess = class({$IFDEF VCL}TWinControl{$ELSE}TWidgetControl{$ENDIF});
...
function GetOperatorKind(AOperator: TcxFilterControlOperator): TcxFilterOperatorKind;
const
  OperatorMap: array[TcxFilterControlOperator] of TcxFilterOperatorKind = (
    ...
    foBetween, foNotBetween, foInList, foNotInList, foYesterday, foToday,
    foTomorrow, foLastWeek, foLastMonth, foLastYear, foThisWeek, foThisMonth,
    foThisYear, foNextWeek, foNextMonth, foNextYear);
begin
  Result := OperatorMap[AOperator];
end;

function GetOperator(AOperatorKind: TcxFilterOperatorKind; AIsNull: Boolean): TcxFilterControlOperator;
const
  OperatorKindMap: array[TcxFilterOperatorKind] of TcxFilterControlOperator = (
    ...
    fcoYesterday, fcoToday, fcoTomorrow, fcoLastWeek, fcoLastMonth, fcoLastYear,
    fcoThisWeek, fcoThisMonth, fcoThisYear, fcoNextWeek, fcoNextMonth, fcoNextYear);
...
begin
  ...
  if (Result in [fcoEqual, fcoNotEqual]) and AIsNull then
    Result := ExtOperatorKindMap[AIsNull and (Result = fcoEqual)];
end;
...
begin
  ...
  TcxFilterControlCriteria(Criteria).Control.CorrectOperatorClass(Result);
end;

function TcxFilterControlCriteriaItem.GetFilter: IcxFilterControl;
begin
  if (Criteria <> nil) and ((Criteria as TcxFilterControlCriteria).Control <> nil) then
    Result := TcxFilterControlCriteria(Criteria).Control.FilterLink
  else
    Result := nil;
end;
...
begin
  ...
  begin
    ...
    if not ViewInfo.Enabled then
      ...
      if (FocusedRow = Self) and (FFocused.HitTest = fhtButton) then
      ...
      begin
        AHightlited := AHotTrack and
          ...
          (HasFocus and (FocusedRow = Condition) and
           (FFocused.HitTest = fhtValue) and (FFocused.ValueIndex = I)));
        FValueEditorStyle.StyleData.FontColor := FontValue.Color;
        ...
        try
          ...
          TcxCustomEditStyleAccess(FValueEditorStyle).ButtonTransparency :=
            AButtonTransparency[AHightlited];
          ...
          if AHightlited then
            FValueEditorStyle.Color := clWindow
          else
            FValueEditorStyle.Color := Color;
          // calculate
          if not TcxCustomFilterEditHelper.EditPropertiesHasButtons(AProperties) then
  ...
  begin
    ...
    begin
      if HasItems and (FItemIndex >= 0) and (FItemIndex < FilterLink.Count) then
        ...
        raise EcxFilterControlError.Create(cxGetResourceString(@cxSFilterErrorBuilding));
    end;
    with TcxFilterCriteriaItem(ACriteriaItem) do
    begin
      FOperator := GetOperator(OperatorKind, ValueIsNull(Value));
      case FOperator of
...
begin
  Result := not (Operator in [fcoBlanks, fcoNonBlanks, fcoYesterday, fcoToday,
    fcoTomorrow, fcoLastWeek, fcoLastMonth, fcoLastYear, fcoThisWeek,
    fcoThisMonth, fcoThisYear, fcoNextWeek, fcoNextMonth, fcoNextYear]);
end;
...
begin
  ...
  begin
    with FilterEditsController do
      FEditorHelper := FindHelper(AProperties.ClassType);
    ...
    if AEditClass <> nil then
      ...
      FEditorProperties := TcxCustomEditProperties(AProperties.ClassType).Create(nil);
    FEditorHelper.InitializeProperties(FEditorProperties, AProperties, True);
    FSupportedOperators := FEditorHelper.GetSupportedFilterOperators(AProperties, True);
    FValueType := FilterLink.ValueTypes[FItemIndex];
    if (FValueType <> nil) and not FValueType.IsString then
      FSupportedOperators := FSupportedOperators - [fcoLike, fcoNotLike];
  end;
end;
...
begin
  ...
  begin
    ...
    InternalInit;
    if (fcoLike in FSupportedOperators) and (FValueType <> nil) and (FValueType.IsString) then
      FOperator := fcoLike
    else
    ...
    if FValueEditor <> nil then
      FInplaceEditors.RemoveItem(TcxCustomEditAccess(FValueEditor).Properties);
  end;
end;
...
begin
  ...
  FFocused.Row := FRoot;
  FFocused.HitTest := fhtBoolOperator;
...
begin
  ...
  case Key of
    ...
    VK_DELETE:
      if ssCtrl in Shift then Remove;
    VK_INSERT: AddCondition(FocusedRow);
  end;
end;
...
begin
  ...
  if Key in [#13, ' '] then
    case FFocused.HitTest of
      ...
      fhtValue: SelectValue(aveEnter, #0);
      fhtAddValue: AddValue;
    end
  else
    if (Key <> #27) and (FFocused.HitTest = fhtValue) then SelectValue(aveKey, Key);
end;
...
begin
  ...
  begin
    FFocused := FHotTrack;
    EnsureRowVisible;
    case FHotTrack.HitTest of
      fhtButton: SelectAction;
      fhtBoolOperator: SelectBoolOperator;
      fhtItem: SelectItem;
      fhtOperator: SelectCondition;
      fhtValue: SelectValue(aveMouse, #0);
      fhtAddCondition: AddCondition(nil); // add last
      fhtAddValue: AddValue;
    end;
  end;
...
begin
  ...
  ViewInfo.GetHitTestInfo([], ScreenToClient(Mouse.CursorPos), FHotTrack);
  LayoutChanged;
end;
...
begin
  ...
  FFocused.Row := TcxConditionViewInfo.Create(Self, ARowParent, nil);
  RecalcRows;
  FFocused.HitTest := fhtAddCondition; // make sure last button visible
  ViewInfo.Calc;
  UpdateScrollBars;
  EnsureRowVisible;
  FFocused.HitTest := fhtItem;
...
begin
  ...
  FFocused.HitTest := fhtValue;
  FFocused.ValueIndex := FocusedRow.Condition.Values.Count - 1;
  LayoutChanged;
end;
...
begin
  if (FFocused.HitTest = fhtValue) and
...
begin
  ...
  while (ARow.Parent <> FRoot) and (ARow.Parent.Group.RowCount = 1) do
    ARow := ARow.Parent;
  FFocused.Row := Rows[FRows.IndexOf(ARow) - 1];
  FreeAndNil(ARow);
...
begin
  FocusedRow.Condition.Values.RemoveValue(FFocused.ValueIndex);
  if FFocused.ValueIndex > 0 then Dec(FFocused.ValueIndex);
  ViewInfo.Calc;
  UpdateScrollBars;
  EnsureRowVisible;
end;
...
begin
  ...
  begin
    if FFocused.HitTest = fhtAddCondition then
    ...
    with ACondition do
      case Operator of
        fcoEqual, fcoNotEqual:
          ACriteriaList.AddItem(ItemLink, GetOperatorKind(Operator),
        ...
        fcoBetween, fcoNotBetween:
          begin
            ...
            ACriteriaList.AddItem(ItemLink, GetOperatorKind(Operator), AValue, AText);
          end;
        fcoInList, fcoNotInList:
          begin
...
begin
  ...
  with TcxComboBoxAccess(FComboBox) do
  begin
    Style.LookAndFeel.MasterLookAndFeel := Self.LookAndFeel;
    Visible := False;
    ParentColor := True;
    Properties.DropDownListStyle := lsFixedList;
    TcxCustomEditPropertiesAccess(Properties).Buttons[0].Visible := False;
    OnExit := ComboBoxExit;
    OnKeyPress := ComboBoxKeyPress;
    OnKeyDown := ComboBoxKeyDown;
    Properties.OnPopup := ComboBoxPopup;
    Properties.OnCloseUp := ComboBoxCloseUp;
  end;
  // style for Values
  FValueEditorStyle := TcxCustomEditStyle.Create(Self, True);
  with TcxCustomEditStyleAccess(FValueEditorStyle) do
  begin
    Font := FontValue;
...
begin
  BuildFromRows;
  if FCriteria.Root.Count > 0 then Result := FCriteria.FilterCaption else Result := '';
end;
...
begin
  BuildFromRows;
  if FCriteria.Root.Count > 0 then Result := FCriteria.FilterText else Result := '';
end;
...
begin
  ...
  begin
    ...
    case AActivateKind of
      aveMouse:
        ActivateByMouse(FFocused.Shift, FFocused.Mouse.X, FFocused.Mouse.Y,
          FocusedRow.Condition.ValueEditorData);
      aveKey: ActivateByKey(AKey, FocusedRow.Condition.ValueEditorData);
    else
      Activate(FocusedRow.Condition.ValueEditorData);
    end;
  end;
end;
...
begin
  Result := FRows.IndexOf(FFocused.Row);
...
begin
  TControlAccess(TcxComboBoxAccess(FComboBox).LookupData.GetActiveControl).Font.Style := [];
end;
...
begin
  if FocusedRow <> Rows[RowCount - 1] then
    if ATab then
    begin
      FFocused.Row := Rows[FocusedRowIndex + 1];
...
begin
  case FFocused.HitTest of
    ...
    fhtValue:
      begin
        if (FocusedRow.Condition.Values.Count - 1) > FFocused.ValueIndex then
        begin
          Inc(FFocused.ValueIndex);
          RowNavigate(fhtValue);
        end
        else
...
begin
  case FFocused.HitTest of
    ...
    fhtValue:
      begin
        if FFocused.ValueIndex > 0 then
        begin
          Dec(FFocused.ValueIndex);
          RowNavigate(fhtValue);
        end
    ...
    begin
      ...
      if FocusedRow is TcxGroupViewInfo then
        ...
        with FocusedRow.Condition do
          if HasDisplayValues then
          begin
            FFocused.ValueIndex := Values.Count - 1;
...
procedure TcxCustomFilterControl.RowNavigate(AElement: TcxFilterControlHitTest);
begin
  FFocused.HitTest := AElement;
  ViewInfo.Calc;
  EnsureRowVisible;
end;
...
begin
  if IsDestroying or IsLoading or not HandleAllocated then Exit;
  if (FComboBox <> nil) and FComboBox.DroppedDown then Exit;
  ViewInfo.Calc;
  CheckVertical;
  CheckHorizontal;
  UpdateScrollBars;
...
procedure TcxCustomFilterControl.ValueEditorInit;
  ...
  var
    ...
    begin
      I := 1;
      S := TcxCustomTextEdit(FValueEditor).Text;
      while I <= Length(S) do
        if S[I] in AWildcardChars then
          Delete(S, I, 1)
        else
          Inc(I);
      TcxCustomTextEdit(FValueEditor).Text := S;
    end;

  begin
    AWildcardChars := [FCriteria.UnderscoreWildcard, FCriteria.PercentWildcard];
    if FValueEditor is TcxCustomTextEdit then
      with TcxCustomTextEditAccess(FValueEditor) do
      begin
...
begin
  ...
  SetValidChars;
  with TcxCustomEditAccess(FValueEditor) do
  begin
  {$IFNDEF VCL}
    Visible := True;
  {$ENDIF}
    ...
    OnFocusChanged := ValueEditorFocusChanged;
...
begin
  ...
  begin
    ...
    with FocusedRow.Condition, Values[FFocused.ValueIndex] do
    begin
      EditorHelper.GetFilterValue(FValueEditor, GetProperties, V, S);
      FilterControlValidateValue(FValueEditor, V, Operator, ValueType);
      FValue := V;
      FValueText := S;
    end;
  end;
...
procedure TcxCustomFilterControl.ValueEditorFocusChanged(Sender: TObject);
begin
//  FocusChanged;
end;
...
{$ENDIF}
  ...
  FPainter := TcxFilterControlPainter.Create(AOwner);
  FButtonState := cxbsNormal;
end;
...
{$IFNDEF VCL}
  Canvas.Canvas.Start;
  try
    TCanvasAccess(Canvas.Canvas).RequiredState([csHandleValid, csFontValid, csBrushValid]);
{$ENDIF}
    with Canvas do
    begin
      Brush.Color := Control.Color;
    ...
    for I := Control.TopVisibleRow to Control.RowCount - 1 do
      ...
      begin
        if Canvas.RectVisible(RowRect) then Painter.DrawRow(Control.Rows[I]);
...
{$ELSE}
  ...
  Control.Canvas.CopyRect(R, Canvas.Canvas, R);
{$ENDIF}
end;
...
begin
  if FLinkComponent <> nil then
    FLinkComponent.GetInterface(IcxFilterControl, Result)
  else
    Result := nil;
end;
...
begin
  if (Value <> nil) and Supports(Value, IcxFilterControl) then
  begin
  ...
  for Y := 0 to 7 do
    ...
    begin
      if (Y mod 2) = (X mod 2) then
        ABitmap.Canvas.Pen.Color := ColorToRGB(clBtnFace)
      else
        ABitmap.Canvas.Pen.Color := ColorToRGB(clBlack);
      ABitmap.Canvas.DrawPoint(X, Y);
    end;
  HalfToneBrush.Bitmap := ABitmap;
  ABitmap.Free;
{$ENDIF}
end;
