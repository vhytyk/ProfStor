...
{$ENDIF}
  ...
  cxDB, cxDataUtils, cxDataStorage, cxCustomData, cxDBData, cxLookupEdit;
...
begin
  if (ARecordIndex <> -1) and (DataController <> nil) then
    FCurrentKey := DataController.GetRecordId(ARecordIndex)
  else
    FCurrentKey := Null;
end;
...
begin
  Result := GetDataField;
  if (Result <> nil) and not Result.Lookup then
    Result := nil;
end;
  ...
  begin
    ...
    try
      ...
      if (ADataSet <> nil) and ADataSet.Active and (AItemIndex <> -1) and
        ADataSet.Locate(AListFieldName, AText, ALocateOptions) then
...
begin
  ...
  begin
    ...
    begin
      try
        ARecordIndex := DataController.FindRecordIndexByKey(AKey);
      except
        on EVariantError do
          ARecordIndex := -1;
      end;
...
begin
  if Properties.ListField <> nil then
    Result := Properties.ListField.IsValidChar(AChar)
  else
    Result := True;
end;
...
begin
  Result := TcxCustomDBLookupEditProperties(InternalGetProperties);
end;
