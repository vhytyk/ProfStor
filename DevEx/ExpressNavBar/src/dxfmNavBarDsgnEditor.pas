...
uses
  Windows, SysUtils, TypInfo, {$IFDEF DELPHI6}Variants,{$ENDIF} Registry, ToolsAPI,
  dxNavBarViewsFact, dxNavBarDsgnConsts;
...
begin
  Result := CanAdd and DsgnWindow.ClipboardComponents {$IFDEF DELPHI5}and DsgnWindow.Designer.CanPaste{$ENDIF};
end;
...
begin
  if AItem.ClassType <> ItemClass then exit;
...
type
  ...
  protected
    ...
    function GetNodeByItem(AItem: TdxNavBarItem): TListItem;
...
function TdxNavBarDsgnWindowLinksHandler.GetNodeByItem(AItem: TdxNavBarItem): TListItem;
var
  I: Integer;
begin
  Result := nil;
  for I := 0 to DsgnWindow.lbxLinkDesignerItems.Items.Count - 1 do
    if TObject(DsgnWindow.lbxLinkDesignerItems.Items[I].Data) = AItem then
    begin
      Result := DsgnWindow.lbxLinkDesignerItems.Items[I];
      break;
    end;
end;
...
function TdxNavBarDsgnWindowDefaultStylesHandler.GetSelectedCount: Integer;
begin
  if MainList <> nil then
    Result := MainList.SelCount
  else Result := 0;
...
begin
  ...
  begin
    if PTypeInfo(TypeInfo(TdxNavBarDefaultStyle))^.Name = PropList[I].PropType^.Name then
    begin
      {$IFDEF DELPHI5}
      V := GetPropValue(NavBar.DefaultStyles, PropList[I].Name);
      {$ELSE}
      PropInfo := GetPropInfo(TypeInfo(TdxNavBarDefaultStyles), PropList[I].Name);
      V := GetOrdProp(NavBar.DefaultStyles, PropInfo);
      {$ENDIF}
...
begin
  ...
  begin
    AComponent := dxNavBarDsgnUtils.TryExtractComponent(Item);
    if (NavBar <> nil) and not (csDestroying in NavBar.ComponentState) and
      (CurrentHandler <> nil) then CurrentHandler.DeleteItem(AComponent);
  end;
end;
...
begin
  inherited;
  Item := dxNavBarDsgnUtils.TryExtractComponent(Component);
  if (NavBar <> nil) and not (csDestroying in NavBar.ComponentState) and
    (CurrentHandler <> nil) then CurrentHandler.DeleteItem(Item);
end;
...
{$ELSE}
procedure TdxfmNavBarDesignWindow.SelectionChanged(ASelection: TComponentList);
begin
  inherited;
  UpdateSelections(ASelection);
end;
...
begin
  ...
  try
    if CurrentHandler is TdxNavBarDsgnWindowComponentCollectionPageHandler then
      Component.Name := NavBar.Name + (CurrentHandler as TdxNavBarDsgnWindowComponentCollectionPageHandler).ItemNamePrefix + IntToStr(I)
    else Component.Name := NavBar.Name + IntToStr(I);
