...
type
  ...
  published
    ...
    property SmallImages: TImageList read FSmallImages write SetSmallImages;
    property ScrollDelay: Integer read FScrollDelay write SetScrollDelay default 300;
...
begin
  if (Owner.Count > 0) and not (csLoading in Owner.ComponentState) then
    raise EdxSideBarError.Create(DXSB_CANTDELETEGROUP)
  else begin
    inherited Clear;
    if not (csLoading in Owner.ComponentState) then
      Add(DXSB_DEFAULTGROUP);
...
begin
  ...
  FCaption := DXSB_DEFAULTITEMCAPTION;
  FLargeImage := -1;
  FSmallImage := -1;
  FEnabled := True;
...
begin
  if Action is TCustomAction then
    ...
    begin
      if not CheckDefaults or (Self.Caption = '') or (Self.Caption = DXSB_DEFAULTITEMCAPTION) then
...
begin
  ...
  FCaption := DXSB_DEFAULTGROUPCAPTION;
  FTopVisibleItem := 0;
  ...
  begin
    ...
    begin
      if (sbmIconType in FOptions) and (Group <> nil) then begin
        InsertNewMenuItem(DXSB_LARGEICONTYPE, True, Group.IconType = dxsgLargeIcon, -101);
        InsertNewMenuItem(DXSB_SMALLICONTYPE, True, Group.IconType = dxsgSmallIcon, -102);
      end;
      ...
      or (sbmRenameGroup in FOptions) then begin
        ...
        if (sbmAddGroup in FOptions) then
          InsertNewMenuItem(DXSB_ADDGROUP, True, False, -201);
        if (sbmRemoveGroup in FOptions) then
          InsertNewMenuItem(DXSB_REMOVEGROUP, Group <> nil, False, -202);
        if (sbmRenameGroup in FOptions) then
          InsertNewMenuItem(DXSB_RENAMEGROUP, Group <> nil, False, -203);
      end;
      ...
      and (Group <> nil) then begin
        if (List.Count > 0)  and (TMenuItem(List.Last).Caption <> '-') then
          InsertNewMenuItem('-', True, False, 0);
        InsertNewMenuItem(DXSB_CUSTOMIZE, True, False, -401);
      end;
    end else begin
      if (sbmRemoveItem in FOptions) then
         InsertNewMenuItem(DXSB_REMOVEITEM, True, False, -301);
      if (sbmRenameItem in FOptions) then
         InsertNewMenuItem(DXSB_RENAMEITEM, True, False, -302);
    end;
