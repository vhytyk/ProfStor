...
uses
  ...
  dxThemeManager, cxClasses, cxGraphics, cxControls, cxLookAndFeels, cxContainer;
...
{$ENDIF}
  ...
  TdxStatusBarKeyboardStateNotifier = class(TThread)
  private
    FTarget: TdxCustomStatusBar;
    FKeys: array of TdxStatusBarKeyboardStateWatchedKey;
  protected
    procedure Execute; override;
  public
    constructor Create(ATarget: TdxCustomStatusBar);
...
begin
  ...
  begin
    ...
    try
      ...
      if TdxStatusBarTextPanelStyle(Source).IsFontStored then
        Font.Assign(TdxStatusBarTextPanelStyle(Source).Font)
      else
        ParentFontChanged;
    finally
      EndUpdate;
...
constructor TdxStatusBarKeyboardStateNotifier.Create(ATarget: TdxCustomStatusBar);
begin
...
procedure TdxStatusBarKeyboardStateNotifier.Execute;
var
  I: Integer;
  ACurState: Integer;
begin
...
begin
  ...
  FNotifier.Priority := tpIdle;
  FNotifier.FreeOnTerminate := True;
...
begin
  inherited Create(AOwner);
  FAlignControl := True;
  if StatusBarControl.ComponentState * [csDesigning, csUpdating, csLoading, csReading] = [csDesigning] then
  begin
    ...
    FContainer.Name := StatusBarControl.Name + 'Container' + IntTostr(Owner.ID);
  end;
end;
...
begin
  ...
  // auto create
  if StatusBarControl.ComponentState * [csDesigning, csLoading, csReading] = [csDesigning] then
  ...
  for I := 0 to Panels.Count - 1 do
    if (Panels[I].PanelStyle is TdxStatusBarTextPanelStyle) and
      TdxStatusBarTextPanelStyle(Panels[I].PanelStyle).AutoHint then
        APanel := Panels[I];
  if (APanel <> nil) and (Action is THintAction) and not DoHint then
  begin
...
const
  AStyles: array[TcxLookAndFeelKind] of TdxStatusBarPaintStyle = (
    stpsStandard, stpsStandard, stpsFlat);
begin
...
begin
  ...
  begin
    ...
    begin
  {$IFNDEF DELPHI6}
      FPoint := ClientToParent(Self, Point(Width, Height), FParentForm);
  {$ELSE}
...
begin
  ...
  ACanvas.Brush.Color := AStatusBar.Color;
  ACanvas.FillRect(Rect(R.Right - GripSize.cx, R.Bottom, R.Right + 1, R.Bottom + 1));
  ACanvas.FillRect(Rect(R.Right, R.Bottom - GripSize.cy, R.Right + 1, R.Bottom));
end;
