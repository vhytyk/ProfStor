...
begin
  ...
  begin
    ...
    DrawColorEdge(ACanvas, R, dxOffice11SelectedBorderColor, etStandard, [epBottom]);
  end
  else
  begin
    R.Top := AActiveTabRect.Top;
    DrawColorEdge(ACanvas, R, dxOffice11SelectedBorderColor, etStandard, [epTop]);
  end;

  if TabContainer.TabsPosition = tctpTop then
    ARect.Top := AActiveTabRect.Bottom
  else ARect.Bottom := AActiveTabRect.Top;
  ACanvas.Brush.Color := ColorToRGB(dxOffice11DownedSelectedColor);
  ACanvas.FillRect(ARect);
end;

procedure TdxDockControlOfficePainter.DrawTab(ACanvas: TCanvas;
...
begin
  ...
  begin
    if TabContainer.TabsPosition = tctpTop then
    begin
      FillGradientRect(ACanvas.Handle, ARect, dxOffice11DownedColor, dxOffice11DownedSelectedColor, False);
      DrawColorEdge(ACanvas, ARect, dxOffice11SelectedBorderColor, etStandard, [epTopLeft, epRight])
    end
    else
    begin
      FillGradientRect(ACanvas.Handle, ARect, dxOffice11DownedSelectedColor, dxOffice11DownedColor, False);
      DrawColorEdge(ACanvas, ARect, dxOffice11SelectedBorderColor, etStandard, [epLeft, epBottomRight]);
    end;
  end
  else
  begin
