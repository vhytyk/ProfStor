...
type
  ...
  TcxPropertiesStoreComponent = class(TCollectionItem, IcxStoredObject, IcxStoredParent)
  private
        ...
        begin
          case APropInfo^.PropType^.Kind of
            ...
            tkSet:
              ...
              {$ELSE}
              {$IFDEF DELPHI5}
              SetSetProp(APersistent, AName, VarToStr(AValue));
              {$ELSE}
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if @TForm(Owner).OnCreate <> @AMyOwnerCreate then
        begin
      ...
      begin
        if @TDataModule(Owner).OnCreate <> @AMyOwnerCreate then
        begin
