...
{$IFDEF VCL}
    procedure lbStylesMeasureItem(Control: TWinControl; Index: Integer;
      var Height: Integer);
{$ELSE}
    procedure lbStylesMeasureItem(Control: TWidgetControl; Index: Integer;
      var Height: Integer);
{$ENDIF}
{$IFDEF VCL}
...
{$ELSE}
    procedure lbStylesDrawItem(Sender: TObject; Index: Integer;
...
begin
  ...
  try
    ...
    {$IFDEF VCL}
    TWinControlAccess(AListBox).RecreateWnd;
    {$ELSE}
    //TODO
    {$ENDIF}
    ...
    for I := 0 to ACount - 1 do
      ...
      begin
       {$IFDEF DELPHI5}
        ...
        {$ENDIF}
        ACacheStyle := TcxStyle.Create(nil);
        if AStyle <> nil then
          ACacheStyle.Assign(AStyle);
        SetOrdProp(FStyleSheet.GetStyles, APropList[I], Integer(ACacheStyle));
...
procedure TfrmcxStyleSheetEditor.lbStylesMeasureItem(Control:
  {$IFDEF VCL}TWinControl{$ELSE}TWidgetControl{$ENDIF};
...
{$ELSE}
procedure TfrmcxStyleSheetEditor.lbStylesDrawItem(Sender: TObject;
...
begin
  ...
  ASelected := odSelected in State;
  Rect.Right := Width;
  if (Index >= 0) and (Index < FStyleList.Count) and not ASelected and
    (svColor in TcxStyle(FStyleList[Index]).AssignedValues) then
    lbStyles.Canvas.Brush.Color := TcxStyle(FStyleList[Index]).Color;
  lbStyles.Canvas.FillRect(Rect);
  if (Index >= 0) and (Index < lbStyles.Items.Count) then
  begin
    {$IFDEF LINUX}
    Inc(Rect.Bottom);
    {$ENDIF}
    ...
    AText := lbStyles.Items[Index];
    TCanvasAccess(lbStyles.Canvas).RequiredState([csHandleValid, csFontValid]);
    {$IFDEF LINUX}
    Inc(Rect.Left, 3);
    {$ENDIF}
    if (svFont in TcxStyle(FStyleList[Index]).AssignedValues) then
    begin
      APrevTextColor := lbStyles.Canvas.Font.Color;
      lbStyles.Canvas.Font.Assign(TcxStyle(FStyleList[Index]).Font);
      lbStyles.Canvas.Font.Color := APrevTextColor;
    end;
    if (svTextColor in TcxStyle(FStyleList[Index]).AssignedValues) and
      not ASelected then
      lbStyles.Canvas.Font.Color := TcxStyle(FStyleList[Index]).TextColor;
    QPainter_drawText(lbStyles.Canvas.Handle, @Rect, AFlags, @AText, -1, nil, nil);
    {$IFDEF LINUX}
    if ASelected or (Index = 0) (*and (ItemIndex = -1) *)then // TODO
    begin
      Dec(Rect.Left, 3);
      lbStyles.Canvas.DrawFocusRect(Rect);
    end;
    {$ENDIF}
  end
end;
...
begin
  ...
  if FStyleSheetPreviews.IndexOf(APreviewClass) < 0 then
    FStyleSheetPreviews.Add(APreviewClass);
end;

procedure UnregisterStyleSheetEditorPreview(APreviewClass: TcxStyleSheetEditorPreviewClass);
begin
  if FStyleSheetPreviews <> nil then
    FStyleSheetPreviews.Remove(APreviewClass);
end;
