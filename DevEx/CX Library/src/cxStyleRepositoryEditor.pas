...
type
  ...
  private
    ...
    procedure SelectItem(AListBox: TListBox; AItem: Pointer);
    function FindItemToSelect(AListBox: TListBox): Pointer;
...
begin
  AItem := StyleRepository.CreateStyleSheetEx(
     TcxCustomStyleSheetClass(TMenuItem(Sender).Tag), StyleRepository.Owner);
...
procedure TcxStyleRepositoryEditor.SelectItem(AListBox: TListBox; AItem: Pointer);
begin
  with AListBox do
...
function TcxStyleRepositoryEditor.FindItemToSelect(AListBox: TListBox): Pointer;
var
  I: Integer;
begin
  Result := nil;
...
begin
  ...
  begin
    AItem := FindItemToSelect(AListBox);
    for I := 0 to AListBox.Items.Count - 1 do
      if AListBox.Selected[I] then
        TcxCustomStyle(AListBox.Items.Objects[I]).Free;
    UpdateItems;
