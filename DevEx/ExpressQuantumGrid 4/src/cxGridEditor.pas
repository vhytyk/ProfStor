...
begin
  inherited;
{$IFNDEF VCL}
  miDeleteView.ShortCut := ShortCut(Key_Delete, []);
{$ENDIF}
end;
...
begin
  Result := {$IFDEF VCL}cxViewEditor{$ELSE}QcxViewEditor{$ENDIF}.GetViewEditorClass(TcxCustomGridViewClass(AView.ClassType));
end;
...
procedure TcxGridEditor.ReadState(Reader: TReader);
var
  AFont: TFont;
begin
  AFont := TFont.Create;
