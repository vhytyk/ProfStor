...
begin
  ...
  if Button = mbRight then
    TcxCustomGrid(GridView.Control).SendNotifications(gnkContextMenu);
end;
...
{$ELSE}
procedure TcxGridSite.SetParent(const AParent: TWidgetControl);
{$ENDIF}
begin
...
{$ELSE}
  ...
  FBoundsChangedTimer := TTimer.Create(nil);
...
begin
  ...
  try
    ...
    if ASelf <> nil then
      ASelf.RemoveListenerLink(@ASelf);
  end;
end;
...
procedure TcxCustomGridStyles.GetDefaultViewParams(Index: Integer; AData: Pointer;
...
procedure TcxCustomGridViewStyles.GetDefaultViewParams(Index: Integer; AData: Pointer;
  out AParams: TcxViewParams);
begin
  inherited;
