...
{$ENDIF}
  ...
  cxClasses, cxGridStrs, cxDataUtils;
...
begin
  ...
  begin
    ...
    AGridExport := GetGridExportClass(AGrid).Create(AFileName,
      AExportType, AGrid.FocusedView{ActiveLevel.GridView});
...
begin
  ...
  begin
    ...
    begin
      if AColor then
        if {$IFDEF VCL}cxStyles{$ELSE}QcxStyles{$ENDIF}.svColor in AStyle.AssignedValues then
          BrushBkColor := cxColorToRGB(AStyle.Color);
      if ATextColor then
        if {$IFDEF VCL}cxStyles{$ELSE}QcxStyles{$ENDIF}.svTextColor in AStyle.AssignedValues then
          FontColor := cxColorToRGB(AStyle.TextColor);
      if AFont then
        if {$IFDEF VCL}cxStyles{$ELSE}QcxStyles{$ENDIF}.svFont in AStyle.AssignedValues then
        begin
          StrPCopy(FontName, AStyle.Font.Name);
...
begin
  ...
  begin
    AStyle := FExport.GetCellStyle(I, ARow)^;
...
begin
  ...
  begin
    AStyle := FExport.GetCellStyle(FCacheColumnCount - 1, I)^;
