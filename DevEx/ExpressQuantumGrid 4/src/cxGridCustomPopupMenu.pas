...
{$IFNDEF DELPHI5}
  cxClasses,
{$ENDIF}
  Classes, SysUtils, Controls, cxGridCustomView, ImgList;
...
uses
  ...
  cxGridRows, cxGridBandedTableView, cxGridCardView, Menus;
...
begin
  ...
  begin
    ...
    try
      ...
      PopupMenu := AMenuInfo.PopupMenu;
      OnPopup := AMenuInfo.OnPopup;
      HitTypes := AMenuInfo.HitTypes;
      GridView := AMenuInfo.GridView;
...
begin
  ...
  if (Operation = opRemove) and Assigned(FPopupMenuInfos) then
  begin
    i := FPopupMenuInfos.Count - 1;
    while i >= 0 do
    begin
      If FPopupMenuInfos[i].PopupMenu = AComponent then
        PopupMenuInfos[i].Free;
      i := i - 1;
    end;
  end;
end;
