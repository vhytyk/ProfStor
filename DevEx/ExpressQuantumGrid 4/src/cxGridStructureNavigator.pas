...
{$ENDIF}
  ...
  cxControls, cxClasses, cxGraphics,
  cxDesignWindows,
...
{$ELSE}
  QEventType_CMDeferUpdate = QEventType(Integer(QEventType_ClxUser) + 1);
{$ENDIF}
...
//    function GetSelectedView: TcxCustomGridView;
  ...
  {$IFDEF VCL}
    procedure CMDeferUpdate(var Message); message CM_DeferUpdate;
  {$ENDIF}
  protected
  {$IFNDEF VCL}
...
{$IFNDEF VCL}
  {$IFDEF MSWINDOWS}
    TcxGridStructureControl(FCustomizeButton).MouseEnter(nil);
  {$ENDIF}
{$ENDIF}  
    Parent := Self;
  end;
...
function TcxGridStructureNavigator.EventFilter(Sender: QObjectH; Event: QEventH): Boolean;
begin
  case QEvent_type(Event) of
    QEventType_CMDeferUpdate:
      begin
...
begin
  ...
  {$ELSE}
  Result := GetParentForm(Grid).DesignerHook as IDesigner;
  {$ENDIF}
end;
...
{$ELSE}
  QApplication_postEvent(Handle, QCustomEvent_create(QEventType_CMDeferUpdate, Self));
{$ENDIF}
end;
